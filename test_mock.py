#!/usr/bin/env python3
"""
Test script for the Mock PDF to JSON Extractor API
"""
import requests
import json
import os
...
def test_mock_pdf_upload(token, pdf_file):
    """Test mock PDF upload with actual file"""
    print(f"\nTesting Mock PDF upload with file: {pdf_file}")
    headers = {"Authorization": f"Bearer {token}"}
    
    try:
        with open(pdf_file, 'rb') as f:
            files = {'file': (pdf_file, f, 'application/pdf')}
            response = requests.post(f"{BASE_URL}/acord-mock/", headers=headers, files=files)
        
        print(f"Upload Status: {response.status_code}")
        print(f"Response: {response.json()}")
        
        if response.status_code == 200:
            print("✅ Mock PDF extraction successful!")
            print("Extracted JSON:")
            print(json.dumps(response.json(), indent=2))
        else:
            print("❌ Mock PDF extraction failed")
...
if __name__ == "__main__":
    print("=== Mock PDF to JSON Extractor API Test ===\n")
    ...
    if token:
        ...
        # Test mock PDF upload
        test_mock_pdf_upload(token, pdf_file)
